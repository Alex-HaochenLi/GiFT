...
from utils import truncate_code_at_stopwords
...
def gen_code(iter, args, eval_data, eval_examples, model, tokenizer):
    ...
    if not os.path.exists(f'./tmp/{args.output_dir}'):
        os.makedirs(f'./tmp/{args.output_dir}', exist_ok=True)
    with open('./tmp/{}/output-{}.json'.format(args.output_dir, args.local_rank), 'w') as f:
        json.dump(output, f)
    ...
    if args.local_rank == 0:
        ...
        save_path = f'./data/{args.difficulty}/selfsample_code'
        if not os.path.exists(save_path):
            os.makedirs(save_path)

        output_list = gather_outputs(args, eval_data, raw=False)

        passed_codes = check_selfsample_code_validity(args, output_list)
...
def gen_sum(iter, args, eval_data, eval_examples, model, tokenizer):
    ...
    if args.local_rank == 0:
        ...
        save_path = f'./data/{args.difficulty}/selfsample_sum'
        if not os.path.exists(save_path):
            os.makedirs(save_path)
...
def gather_outputs(args, eval_data, raw):
    final_output = {}
    for i in range(dist.get_world_size()):
        # read generated codes
        if not raw:
            file_path = os.path.join(f'./tmp/{args.output_dir}', 'output-{}.json'.format(i))
        else:
            file_path = os.path.join(f'./tmp/{args.output_dir}', 'raw-output-{}.json'.format(i))
...
def load_test(args):
    ...
    tests = []
    for item in data:
        tmp = {'fn_name': item['test']['fn_name'], 'inputs': item['test']['inputs'][:10], 'outputs': item['test']['outputs'][:10]}
        tests.append(tmp)
    std_based = [item['std_based'] for item in data]
    return tests, std_based


def check_selfsample_code_validity(args, codes):
    tests, std_based = load_test(args)
    assert len(codes) == len(tests)

    tests_list = [element for i, element in enumerate(tests) for _ in range(len(codes[i]))]
    std_based_list = [element for i, element in enumerate(std_based) for _ in range(len(codes[i]))]
    ...
    test_cases = [{'test': test, 'code': code, 'std_sign': std_sign}
                  for test, code, std_sign in zip(tests_list, codes_list, std_based_list)]
    with concurrent.futures.ProcessPoolExecutor(max_workers=8) as executor:
        result_list = list(executor.map(parallel_run_test, test_cases))
...
def load_gen_data(iter, args, tokenizer, split_tag='train'):
    with open(f'./data/apps+valid-{args.difficulty}.json', 'r') as f:
        data = json.load(f)

    if iter != 0:
        path = f'./data/{args.difficulty}/selfsample_sum'
...
def load_gibbs_data(iter, args, tokenizer):
    ...
    with open(f'./data/ict_data_{args.difficulty}.json', 'r') as f:
    ...
    path = f'./data/{args.difficulty}/selfsample_code'
    path = os.path.join(path, args.model_name_or_path)
