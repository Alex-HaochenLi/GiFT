...
def check_selfsample_code_validity(codes):
    ...
    tests_list = [element for i, element in enumerate(train_dataset) for _ in range(len(codes[i]))]
    codes_list = [code for item in codes for code in item]
    assert len(codes_list) == len(tests_list)

    test_cases = [{'test': test, 'code': code} for test, code in zip(tests_list, codes_list)]
    ...
    for sublist in codes:
        length = len(sublist)
        transformed_result.append(result_list[index:index + length])
        index += length
...
def worker_function(test_case, queue):
    result = test_case['test'].test(test_case['code'], timeout_duration=5)
    queue.put(result)
